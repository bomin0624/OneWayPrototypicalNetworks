...
class PrototypicalNetwork(nn.Module):
    def __init__(self, device):
        ...
        self.std = 1.0
        self.m = normal.Normal(
            torch.tensor([0.0]).to(device), torch.tensor([self.std]).to(device)
        )
    ...
    def forward(self, batch, device):
        ...
        distances_mean_queries = torch.norm(
            mean_support_embedding - query_embeddings, dim=1
        )
    ...
    def forward_eval(self, batch, device):
        with torch.no_grad():
            ...
            # normal distribution
            probs = torch.exp(-(distances_mean_queries * distances_mean_queries / 2.0))
            probs = torch.max(probs, torch.tensor(1e-6).to(device))
            probs = torch.min(probs, torch.tensor(1.0 - (1e-6)).to(device))
