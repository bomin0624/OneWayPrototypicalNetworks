...
class PrototypicalNetworkDataLoader:
    ...
    def __iter__(self):
        ...
        self.ds.train_data_no = self.ds.train_data_no.sample(frac=1, ignore_index=True)
        num_of_support = 5
        num_of_query = 45
        ...
        n_episode = (
            min(len(self.ds.train_data_yes), len(self.ds.train_data_no)) // nq_no
        )

        for i in range(n_episode):
            support_set = self.ds.train_data_yes[nq_no * i : nq_no * i + num_of_support]
            query_set_yes = self.ds.train_data_yes[
                nq_no * i + num_of_support : nq_no * (i + 1)
            ]
...
class EvalPrototypicalNetworkDataLoader:
    ...
    def get_batch(self, mode="val"):
        ...
        if mode == "val":
            ...
            query_set = pd.concat([self.ds.test_data_yes, self.ds.test_data_no])
        batch = [support_set, query_set]
        return batch
